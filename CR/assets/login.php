...
if (isLoggedIn()) {
    $check = $pdo->prepare('SELECT role,role2 FROM users WHERE id=?');
    $check->execute([(int)$_SESSION['user_id']]);
    $access = $check->fetch(PDO::FETCH_ASSOC);
    if (($access['role2'] ?? '') === ASSET_ROLE) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($username === '' || $password === '') $message = 'Username and password are required.';
        ...
        $stmt = $pdo->prepare('SELECT id,username,password,role,role2 FROM users WHERE username=? LIMIT 1');
        ...
        elseif (!hash_equals(ASSET_ROLE, (string)($user['role2'] ?? '')) && !hash_equals('admin', (string)($user['role'] ?? ''))) $message = 'Your account does not have access to Asset Management.';
            ...
            header('Location: ' . (($user['role2'] ?? '') === ASSET_ROLE ? 'index.php' : 'users.php'));
            exit;
        }
    }
}
