...
final class PdfScanner
{
    public static function scan(string $path, string $prefix = ''): array
    {
        ...
        $number = self::assetNumber($text, $prefix);
        $date = self::testDate($text);
    ...
    private static function extractText(string $path): string
    {
        ...
        if (substr($binary, 0, 4) !== '%PDF') return '';
        $tool = function_exists('shell_exec') ? trim((string)shell_exec('command -v pdftotext 2>/dev/null')) : '';
        if ($tool !== '') {
            $command = escapeshellarg($tool) . ' -layout -nopgbrk ' . escapeshellarg($path) . ' - 2>/dev/null';
            $text = (string)shell_exec($command);
            if (trim($text) !== '') return $text;
        }
        $chunks = [$binary];
        if (preg_match_all('/stream\R(.*?)\Rendstream/s', $binary, $streams)) foreach ($streams[1] as $stream) {
            $decoded = @gzuncompress($stream);
            if ($decoded === false) $decoded = @gzinflate($stream);
            if ($decoded === false) $decoded = @gzdecode($stream);
            if ($decoded !== false) $chunks[] = $decoded;
        }
        $text = '';
        foreach ($chunks as $chunk) if (preg_match_all('/\((?:\\.|[^\\)])*\)/s', $chunk, $matches)) foreach ($matches[0] as $item) $text .= ' ' . stripcslashes(substr($item, 1, -1));
    ...
    private static function assetNumber(string $text, string $prefix): string
    {
        ...
        $patterns = [];
        if ($prefix !== '') $patterns[] = '/\b(' . preg_quote($prefix, '/') . '[\s._-]*\d(?:[\s._-]*\d){2,7})\b/i';
        if ($prefix === '') $patterns[] = '/Customer\s+Asset\s+(?:Number|No\.?|#)?\s*[:#-]?\s*([A-Z]{1,6}[- ]?\d{3,8})\b/i';
        foreach ($patterns as $pattern) if (preg_match($pattern, $text, $match)) return strtoupper(preg_replace('/[^A-Z0-9]/i', '', $match[1] ?? $match[0]) ?? '');
        return '';
    }
    private static function testDate(string $text): string
    {
        ...
        foreach ($patterns as $pattern) {
            ...
            if (preg_match_all($pattern, $text, $matches)) foreach ($matches[1] as $value) {
                $date = self::normaliseDate($value);
                if ($date !== '' && $date <= $today) $valid[] = $date;
            }
            if ($valid) {
    ...
    private static function normaliseDate(string $value): string
    {
        ...
        foreach (['!d/m/Y', '!d.m.Y', '!d-m-Y', '!Y-m-d', '!d/m/y', '!d.m.y', '!d-m-y', '!j F Y', '!j M Y'] as $format) {
