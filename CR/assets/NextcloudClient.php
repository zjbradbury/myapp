...
final class NextcloudClient
{
    ...
    public function upload(string $localFile, string $originalName, string $assetNumber, bool $useExactName = false): string
    {
        ...
        try {
            $this->request('PUT', $path, $handle, (int)filesize($localFile), [200, 201, 204]);
        } finally {
            fclose($handle);
        }
        return $path;
    ...
    {
        $this->request('GET', $path, null, null, [200], static function ($curl, string $data): int {
            echo $data;
            return strlen($data);
        });
    }
    public function downloadToFile(string $path, string $localFile): void
    {
        ...
        if ($handle === false) throw new RuntimeException('Unable to create a temporary download file.');
        try {
            $this->request('GET', $path, null, null, [200], static function ($curl, string $data) use ($handle): int {
    ...
    {
        $this->request('DELETE', $path, null, null, [200, 204, 404]);
    }
